...
interface InvoiceIntegrationClient {
    ...
    class MockClient(
                        ...
                        outputStream.bufferedWriter(cp1252Charset).use { writer ->
                            invoices.forEach { invoice ->
                                ...
                                invoice.rows.forEach { row ->
                                    val rowLine =
                                        formatInvoiceRow(invoice.headOfFamily.ssn.toString(), row)
                                    writer.write(rowLine)
                                    writer.newLine()
                                    val descriptionLine =
                                        formatServiceNeedDescription(
                                            invoice.headOfFamily.ssn.toString(),
                                            row,
                                        )
                                    writer.write(descriptionLine)
                                    writer.newLine()
                                    val locationTimeLine =
                                        formatLocationTimeDescription(
                                            invoice.headOfFamily.ssn.toString(),
                                            row,
                                        )
        ...
        private fun formatInvoiceHeaderLine(invoice: InvoiceDetailed): String {
            ...
            // setField(line, 64, 104, "${invoice.headOfFamily.firstName}
            // ${invoice.headOfFamily.lastName}") // L5
        ...
        private fun formatInvoiceRow(ssn: String, row: InvoiceRowDetailed): String {
            ...
            val creditPosting =
                if (row.unitName == "Kintauden päiväkoti") "32950552013021"
                else "32950552023021" // TODO: handle with unit ID
        ...
        private fun formatServiceNeedDescription(ssn: String, row: InvoiceRowDetailed): String {
            ...
            val description =
                "${row.child.firstName} ${row.child.lastName} ${row.serviceNeed ?: ""}"
