// SPDX-FileCopyrightText: 2017-2026 City of Espoo
//
// SPDX-License-Identifier: LGPL-2.1-or-later

// GENERATED FILE: no manual modifications

export namespace Action {

export type Global =
  | 'APPLICATIONS_PAGE'
  | 'CREATE_CLUB_TERM'
  | 'CREATE_DOCUMENT_TEMPLATE'
  | 'CREATE_DRAFT_INVOICES'
  | 'CREATE_DRAFT_PAYMENTS'
  | 'CREATE_EMPLOYEE'
  | 'CREATE_FEE_THRESHOLDS'
  | 'CREATE_HOLIDAY_PERIOD'
  | 'CREATE_HOLIDAY_QUESTIONNAIRE'
  | 'CREATE_PAPER_APPLICATION'
  | 'CREATE_PERSON'
  | 'CREATE_PERSONAL_MOBILE_DEVICE_PAIRING'
  | 'CREATE_PERSON_FROM_VTJ'
  | 'CREATE_PRESCHOOL_TERM'
  | 'CREATE_UNIT'
  | 'CREATE_VOUCHER_VALUE'
  | 'DELETE_HOLIDAY_PERIOD'
  | 'DELETE_HOLIDAY_QUESTIONNAIRE'
  | 'DELETE_VOUCHER_VALUE'
  | 'DOCUMENT_TEMPLATES_PAGE'
  | 'EMPLOYEES_PAGE'
  | 'FETCH_INCOME_STATEMENTS_AWAITING_HANDLER'
  | 'FINANCE_BASICS_PAGE'
  | 'FINANCE_PAGE'
  | 'HOLIDAY_AND_TERM_PERIODS_PAGE'
  | 'MESSAGES_PAGE'
  | 'OUT_OF_OFFICE_PAGE'
  | 'PERSONAL_MOBILE_DEVICE_PAGE'
  | 'PERSON_SEARCH_PAGE'
  | 'PIN_CODE_PAGE'
  | 'PLACEMENT_TOOL'
  | 'READ_ACTIVE_HOLIDAY_QUESTIONNAIRES'
  | 'READ_AROMI_ORDERS'
  | 'READ_ASSISTANCE_ACTION_OPTIONS'
  | 'READ_CHILD_DOCUMENT_DECISIONS_REPORT'
  | 'READ_CUSTOMER_FEES_REPORT'
  | 'READ_DECISIONS_REPORT'
  | 'READ_DECISION_REASONINGS'
  | 'READ_DECISION_UNITS'
  | 'READ_DOCUMENT_TEMPLATE'
  | 'READ_DRAFT_OCCUPANCIES'
  | 'READ_DUPLICATE_PEOPLE_REPORT'
  | 'READ_EMPLOYEES'
  | 'READ_ENDED_PLACEMENTS_REPORT'
  | 'READ_FEE_THRESHOLDS'
  | 'READ_FINANCE_DECISION_HANDLERS'
  | 'READ_FUTURE_PRESCHOOLERS'
  | 'READ_HOLIDAY_PERIOD'
  | 'READ_HOLIDAY_PERIODS'
  | 'READ_HOLIDAY_QUESTIONNAIRE'
  | 'READ_HOLIDAY_QUESTIONNAIRES'
  | 'READ_INCOME_COEFFICIENT_MULTIPLIERS'
  | 'READ_INCOME_TYPES'
  | 'READ_INCOMPLETE_INCOMES_REPORT'
  | 'READ_INVOICE_CODES'
  | 'READ_INVOICE_REPORT'
  | 'READ_MISSING_HEAD_OF_FAMILY_REPORT'
  | 'READ_NON_SSN_CHILDREN_REPORT'
  | 'READ_PERSONAL_MOBILE_DEVICES'
  | 'READ_PLACEMENT_COUNT_REPORT'
  | 'READ_PLACEMENT_DESKTOP_DAYCARES'
  | 'READ_PLACEMENT_SKETCHING_REPORT'
  | 'READ_PRESCHOOL_ABSENCE_REPORT_FOR_AREA'
  | 'READ_RAW_REPORT'
  | 'READ_SELECTABLE_FINANCE_DECISION_HANDLERS'
  | 'READ_SERVICE_NEED_OPTIONS'
  | 'READ_SERVICE_WORKER_APPLICATION_NOTES'
  | 'READ_SEXTET_REPORT'
  | 'READ_SPECIAL_DIET_LIST'
  | 'READ_SYSTEM_NOTIFICATIONS'
  | 'READ_TAMPERE_REGIONAL_SURVEY_REPORT'
  | 'READ_UNITS'
  | 'READ_UNITS_REPORT'
  | 'READ_UNIT_FEATURES'
  | 'READ_VARDA_REPORT'
  | 'READ_VOUCHER_VALUES'
  | 'REPORTS_PAGE'
  | 'SEARCH_EMPLOYEES'
  | 'SEARCH_FEE_DECISIONS'
  | 'SEARCH_INVOICES'
  | 'SEARCH_PAYMENTS'
  | 'SEARCH_PEOPLE'
  | 'SEARCH_PEOPLE_UNRESTRICTED'
  | 'SEARCH_VOUCHER_VALUE_DECISIONS'
  | 'SEND_JAMIX_ORDERS'
  | 'SEND_PATU_REPORT'
  | 'SETTINGS_PAGE'
  | 'SHIFT_PLANNING_PAGE'
  | 'SHIFT_WISH_PAGE'
  | 'SUBMIT_PATU_REPORT'
  | 'UNITS_PAGE'
  | 'UNIT_FEATURES_PAGE'
  | 'UPDATE_HOLIDAY_PERIOD'
  | 'UPDATE_HOLIDAY_QUESTIONNAIRE'
  | 'UPDATE_SETTINGS'
  | 'UPDATE_SYSTEM_NOTIFICATION'
  | 'UPDATE_VOUCHER_VALUE'
  | 'VARDA_OPERATIONS'
  | 'WRITE_DECISION_REASONINGS'
  | 'WRITE_SERVICE_WORKER_APPLICATION_NOTES'

export type AbsenceApplication =
  | 'DECIDE'
  | 'DECIDE_MAX_WEEK'
  | 'READ'

export type Application =
  | 'ACCEPT_DECISION'
  | 'CANCEL'
  | 'CANCEL_PLACEMENT_PLAN'
  | 'CONFIRM_DECISIONS_MAILED'
  | 'CREATE_NOTE'
  | 'CREATE_PLACEMENT_PLAN'
  | 'MOVE_TO_WAITING_PLACEMENT'
  | 'READ'
  | 'READ_DECISION_DRAFT'
  | 'READ_IF_HAS_ASSISTANCE_NEED'
  | 'READ_METADATA'
  | 'READ_NOTES'
  | 'READ_PLACEMENT_PLAN_DRAFT'
  | 'READ_SERVICE_WORKER_ACCOUNT_MESSAGES'
  | 'READ_SPECIAL_EDUCATION_TEACHER_NOTES'
  | 'REJECT_DECISION'
  | 'RESPOND_TO_PLACEMENT_PROPOSAL'
  | 'RETURN_TO_SENT'
  | 'SEND'
  | 'SEND_DECISIONS_WITHOUT_PROPOSAL'
  | 'SEND_PLACEMENT_PROPOSAL'
  | 'UPDATE'
  | 'UPDATE_DECISION_DRAFT'
  | 'UPDATE_PLACEMENT_DRAFT'
  | 'UPLOAD_ATTACHMENT'
  | 'VERIFY'
  | 'WITHDRAW_PLACEMENT_PROPOSAL'

export type ApplicationNote =
  | 'DELETE'
  | 'UPDATE'

export type AssistanceAction =
  | 'DELETE'
  | 'READ'
  | 'UPDATE'

export type AssistanceFactor =
  | 'DELETE'
  | 'READ'
  | 'UPDATE'

export type AssistanceNeedVoucherCoefficient =
  | 'DELETE'
  | 'UPDATE'

export type BackupCare =
  | 'DELETE'
  | 'UPDATE'

export type BackupPickup =
  | 'DELETE'
  | 'UPDATE'

export type Child =
  | 'CREATE_ABSENCE'
  | 'CREATE_ASSISTANCE_ACTION'
  | 'CREATE_ASSISTANCE_FACTOR'
  | 'CREATE_ASSISTANCE_NEED_VOUCHER_COEFFICIENT'
  | 'CREATE_ATTENDANCE_RESERVATION'
  | 'CREATE_BACKUP_CARE'
  | 'CREATE_BACKUP_PICKUP'
  | 'CREATE_CHILD_DECISION_DOCUMENT'
  | 'CREATE_CHILD_DOCUMENT'
  | 'CREATE_DAILY_NOTE'
  | 'CREATE_DAILY_SERVICE_TIME'
  | 'CREATE_DAYCARE_ASSISTANCE'
  | 'CREATE_FEE_ALTERATION'
  | 'CREATE_OTHER_ASSISTANCE_MEASURE'
  | 'CREATE_PEDAGOGICAL_DOCUMENT'
  | 'CREATE_PRESCHOOL_ASSISTANCE'
  | 'CREATE_STICKY_NOTE'
  | 'DELETE_ABSENCE'
  | 'DELETE_ABSENCE_RANGE'
  | 'DELETE_HOLIDAY_RESERVATIONS'
  | 'DELETE_IMAGE'
  | 'READ'
  | 'READ_ABSENCES'
  | 'READ_ABSENCE_APPLICATIONS'
  | 'READ_ADDITIONAL_INFO'
  | 'READ_APPLICATION'
  | 'READ_ASSISTANCE'
  | 'READ_ASSISTANCE_ACTION'
  | 'READ_ASSISTANCE_FACTORS'
  | 'READ_ASSISTANCE_NEED_VOUCHER_COEFFICIENTS'
  | 'READ_ATTENDANCE_REPORT'
  | 'READ_BACKUP_CARE'
  | 'READ_BACKUP_PICKUP'
  | 'READ_BASIC_INFO'
  | 'READ_BLOCKED_GUARDIANS'
  | 'READ_CHILD_DOCUMENT'
  | 'READ_DAILY_SERVICE_TIMES'
  | 'READ_DAYCARE_ASSISTANCES'
  | 'READ_FAMILY_CONTACTS'
  | 'READ_FEE_ALTERATIONS'
  | 'READ_FUTURE_ABSENCES'
  | 'READ_GUARDIANS'
  | 'READ_NON_RESERVABLE_RESERVATIONS'
  | 'READ_ONGOING_ATTENDANCE'
  | 'READ_OTHER_ASSISTANCE_MEASURES'
  | 'READ_PEDAGOGICAL_DOCUMENTS'
  | 'READ_PLACEMENT'
  | 'READ_PRESCHOOL_ASSISTANCES'
  | 'READ_SENSITIVE_INFO'
  | 'READ_SERVICE_APPLICATIONS'
  | 'READ_SERVICE_NEEDS'
  | 'UPDATE_ADDITIONAL_INFO'
  | 'UPDATE_FAMILY_CONTACT_DETAILS'
  | 'UPDATE_FAMILY_CONTACT_PRIORITY'
  | 'UPDATE_NON_RESERVABLE_RESERVATIONS'
  | 'UPLOAD_IMAGE'
  | 'UPSERT_CHILD_DATE_PRESENCE'

export type ChildDailyNote =
  | 'DELETE'
  | 'UPDATE'

export type ChildDocument =
  | 'ACCEPT_DECISION'
  | 'ANNUL_DECISION'
  | 'ARCHIVE'
  | 'DELETE'
  | 'DOWNLOAD'
  | 'DOWNLOAD_VERSION'
  | 'NEXT_STATUS'
  | 'PREV_STATUS'
  | 'PROPOSE_DECISION'
  | 'PUBLISH'
  | 'READ'
  | 'READ_ACCEPTED_DECISIONS'
  | 'READ_METADATA'
  | 'REJECT_DECISION'
  | 'UPDATE'
  | 'UPDATE_DECISION_VALIDITY'

export type DailyServiceTime =
  | 'DELETE'
  | 'UPDATE'

export type DaycareAssistance =
  | 'DELETE'
  | 'READ'
  | 'UPDATE'

export type Decision =
  | 'ARCHIVE'
  | 'DOWNLOAD_PDF'
  | 'READ'

export type FeeAlteration =
  | 'DELETE'
  | 'UPDATE'
  | 'UPLOAD_ATTACHMENT'

export type FeeDecision =
  | 'ARCHIVE'
  | 'IGNORE'
  | 'READ'
  | 'READ_METADATA'
  | 'UNIGNORE'
  | 'UPDATE'

export type FinanceNote =
  | 'DELETE'
  | 'UPDATE'

export type Group =
  | 'CREATE_ABSENCES'
  | 'CREATE_CALENDAR_EVENT'
  | 'CREATE_CARETAKERS'
  | 'CREATE_CHILD_DOCUMENTS'
  | 'CREATE_NOTE'
  | 'DELETE'
  | 'DELETE_ABSENCES'
  | 'DELETE_CARETAKERS'
  | 'MARK_ARRIVAL'
  | 'MARK_DEPARTURE'
  | 'MARK_EXTERNAL_ARRIVAL'
  | 'MARK_EXTERNAL_DEPARTURE'
  | 'NEKKU_MANUAL_ORDER'
  | 'READ_ABSENCES'
  | 'READ_CARETAKERS'
  | 'READ_CITIZEN_DOCUMENT_RESPONSE_REPORT'
  | 'READ_NOTES'
  | 'READ_STAFF_ATTENDANCES'
  | 'RECEIVE_PUSH_NOTIFICATIONS'
  | 'UPDATE'
  | 'UPDATE_CARETAKERS'
  | 'UPDATE_STAFF_ATTENDANCES'

export type GroupPlacement =
  | 'DELETE'
  | 'UPDATE'

export type Income =
  | 'DELETE'
  | 'UPDATE'
  | 'UPLOAD_ATTACHMENT'

export type Invoice =
  | 'DELETE'
  | 'MARK_SENT'
  | 'READ'
  | 'RESEND'
  | 'SEND'
  | 'UPLOAD_ATTACHMENT'

export type InvoiceCorrection =
  | 'DELETE'
  | 'UPDATE_NOTE'

export type MobileDevice =
  | 'DELETE'
  | 'UPDATE_NAME'

export type OtherAssistanceMeasure =
  | 'DELETE'
  | 'READ'
  | 'UPDATE'

export type Pairing = 'POST_RESPONSE'

export type Parentship =
  | 'DELETE'
  | 'DELETE_CONFLICTED_PARENTSHIP'
  | 'RETRY'
  | 'UPDATE'

export type Partnership =
  | 'DELETE'
  | 'RETRY'
  | 'UPDATE'

export type Payment =
  | 'CONFIRM'
  | 'DELETE'
  | 'REVERT_TO_DRAFT'
  | 'SEND'

export type Person =
  | 'ADD_SSN'
  | 'CREATE_FINANCE_NOTE'
  | 'CREATE_FOSTER_PARENT_RELATIONSHIP'
  | 'CREATE_INCOME'
  | 'CREATE_INVOICE_CORRECTION'
  | 'CREATE_PARENTSHIP'
  | 'CREATE_PARTNERSHIP'
  | 'CREATE_REPLACEMENT_DRAFT_INVOICES'
  | 'DELETE'
  | 'DISABLE_SSN_ADDING'
  | 'DOWNLOAD_ADDRESS_PAGE'
  | 'ENABLE_SSN_ADDING'
  | 'GENERATE_RETROACTIVE_FEE_DECISIONS'
  | 'GENERATE_RETROACTIVE_VOUCHER_VALUE_DECISIONS'
  | 'MERGE'
  | 'READ'
  | 'READ_APPLICATIONS'
  | 'READ_CHILD_PLACEMENT_PERIODS'
  | 'READ_DECISIONS'
  | 'READ_DEPENDANTS'
  | 'READ_FAMILY_OVERVIEW'
  | 'READ_FEE_DECISIONS'
  | 'READ_FINANCE_NOTES'
  | 'READ_FOSTER_CHILDREN'
  | 'READ_FOSTER_PARENTS'
  | 'READ_INCOME'
  | 'READ_INCOME_NOTIFICATIONS'
  | 'READ_INCOME_STATEMENTS'
  | 'READ_INVOICES'
  | 'READ_INVOICE_ADDRESS'
  | 'READ_INVOICE_CORRECTIONS'
  | 'READ_OPH_OID'
  | 'READ_PARENTSHIPS'
  | 'READ_PARTNERSHIPS'
  | 'READ_TIMELINE'
  | 'READ_VOUCHER_VALUE_DECISIONS'
  | 'UPDATE'
  | 'UPDATE_EVAKA_RIGHTS'
  | 'UPDATE_FROM_VTJ'
  | 'UPDATE_INVOICE_ADDRESS'
  | 'UPDATE_OPH_OID'
  | 'UPDATE_PERSONAL_DETAILS'

export type Placement =
  | 'CREATE_GROUP_PLACEMENT'
  | 'CREATE_SERVICE_NEED'
  | 'DELETE'
  | 'UPDATE'

export type PreschoolAssistance =
  | 'DELETE'
  | 'READ'
  | 'UPDATE'

export type ServiceApplication =
  | 'ACCEPT'
  | 'REJECT'

export type ServiceNeed =
  | 'DELETE'
  | 'UPDATE'

export type Unit =
  | 'ACCEPT_PLACEMENT_PROPOSAL'
  | 'CREATE_CALENDAR_EVENT'
  | 'CREATE_GROUP'
  | 'CREATE_MOBILE_DEVICE_PAIRING'
  | 'CREATE_PLACEMENT'
  | 'CREATE_SHIFT_WISH'
  | 'CREATE_TEMPORARY_EMPLOYEE'
  | 'DELETE_TEMPORARY_EMPLOYEE'
  | 'DOWNLOAD_SHIFT_PLAN_PDF'
  | 'INSERT_ACL_EARLY_CHILDHOOD_EDUCATION_SECRETARY'
  | 'INSERT_ACL_SPECIAL_EDUCATION_TEACHER'
  | 'INSERT_ACL_STAFF'
  | 'INSERT_ACL_TYOVUOROSUUNNITTELIJA'
  | 'INSERT_ACL_UNIT_SUPERVISOR'
  | 'READ'
  | 'READ_ABSENCE_APPLICATIONS'
  | 'READ_ACL'
  | 'READ_APPLICATIONS_AND_PLACEMENT_PLANS'
  | 'READ_APPLICATIONS_REPORT'
  | 'READ_ASSISTANCE_NEEDS_AND_ACTIONS_REPORT'
  | 'READ_ASSISTANCE_NEEDS_AND_ACTIONS_REPORT_BY_CHILD'
  | 'READ_ATTENDANCES'
  | 'READ_ATTENDANCE_RESERVATIONS'
  | 'READ_ATTENDANCE_RESERVATION_REPORT'
  | 'READ_CALENDAR_EVENTS'
  | 'READ_CHILD_AGE_AND_LANGUAGE_REPORT'
  | 'READ_CHILD_ATTENDANCES'
  | 'READ_CHILD_CAPACITY_FACTORS'
  | 'READ_CHILD_DOCUMENTS_REPORT'
  | 'READ_CHILD_IN_DIFFERENT_ADDRESS_REPORT'
  | 'READ_CHILD_RESERVATIONS'
  | 'READ_EXCEEDED_SERVICE_NEEDS_REPORT'
  | 'READ_FAMILY_CONFLICT_REPORT'
  | 'READ_FAMILY_CONTACT_REPORT'
  | 'READ_FAMILY_DAYCARE_MEAL_REPORT'
  | 'READ_GROUPS'
  | 'READ_GROUP_DETAILS'
  | 'READ_HOLIDAY_PERIOD_ATTENDANCE_REPORT'
  | 'READ_HOLIDAY_QUESTIONNAIRE_REPORT'
  | 'READ_MEAL_REPORT'
  | 'READ_MISSING_GROUP_PLACEMENTS'
  | 'READ_MISSING_SERVICE_NEED_REPORT'
  | 'READ_MOBILE_DEVICES'
  | 'READ_MOBILE_INFO'
  | 'READ_MOBILE_STATS'
  | 'READ_NEKKU_ORDER_REPORT'
  | 'READ_OCCUPANCIES'
  | 'READ_OCCUPANCY_REPORT'
  | 'READ_PARTNERS_IN_DIFFERENT_ADDRESS_REPORT'
  | 'READ_PLACEMENT_GUARANTEE_REPORT'
  | 'READ_PRESCHOOL_ABSENCE_REPORT_FOR_UNIT'
  | 'READ_PRESCHOOL_APPLICATION_REPORT'
  | 'READ_REALTIME_STAFF_ATTENDANCES'
  | 'READ_SERVICE_APPLICATIONS'
  | 'READ_SERVICE_NEED_REPORT'
  | 'READ_SERVICE_VOUCHER_REPORT'
  | 'READ_SERVICE_VOUCHER_VALUES_REPORT'
  | 'READ_SERVICE_WORKER_NOTE'
  | 'READ_SHIFT_PLAN'
  | 'READ_STAFF_ATTENDANCES'
  | 'READ_STAFF_EMPLOYEE_NUMBER'
  | 'READ_STAFF_OCCUPANCY_COEFFICIENTS'
  | 'READ_STARTING_PLACEMENTS_REPORT'
  | 'READ_TEMPORARY_EMPLOYEE'
  | 'READ_TERMINATED_PLACEMENTS'
  | 'READ_TITANIA_ERRORS'
  | 'READ_TRANSFER_APPLICATIONS'
  | 'READ_UNIT_RESERVATION_STATISTICS'
  | 'READ_UNREAD_MESSAGES'
  | 'SET_SERVICE_WORKER_NOTE'
  | 'UPDATE'
  | 'UPDATE_ACL_EARLY_CHILDHOOD_EDUCATION_SECRETARY'
  | 'UPDATE_ACL_SCHEDULED'
  | 'UPDATE_ACL_SPECIAL_EDUCATION_TEACHER'
  | 'UPDATE_ACL_STAFF'
  | 'UPDATE_ACL_TYOVUOROSUUNNITTELIJA'
  | 'UPDATE_ACL_UNIT_SUPERVISOR'
  | 'UPDATE_CHILD_ATTENDANCES'
  | 'UPDATE_FEATURES'
  | 'UPDATE_SHIFT_PLAN'
  | 'UPDATE_SHIFT_WISH'
  | 'UPDATE_STAFF_ATTENDANCES'
  | 'UPDATE_STAFF_GROUP_ACL'
  | 'UPDATE_TEMPORARY_EMPLOYEE'
  | 'UPSERT_STAFF_OCCUPANCY_COEFFICIENTS'

export type VoucherValueDecision =
  | 'ARCHIVE'
  | 'IGNORE'
  | 'READ'
  | 'READ_METADATA'
  | 'UNIGNORE'
  | 'UPDATE'

export namespace Citizen {

export type AbsenceApplication = 'DELETE'

export type Application =
  | 'DELETE'
  | 'READ'
  | 'READ_DECISIONS'
  | 'UPDATE'
  | 'UPLOAD_ATTACHMENT'

export type Child =
  | 'CREATE_ABSENCE'
  | 'CREATE_ABSENCE_APPLICATION'
  | 'CREATE_APPLICATION'
  | 'CREATE_CALENDAR_EVENT_TIME_RESERVATION'
  | 'CREATE_HOLIDAY_ABSENCE'
  | 'CREATE_INCOME_STATEMENT'
  | 'CREATE_RESERVATION'
  | 'CREATE_SERVICE_APPLICATION'
  | 'READ_ABSENCE_APPLICATIONS'
  | 'READ_ATTENDANCE_SUMMARY'
  | 'READ_CHILD_DOCUMENTS'
  | 'READ_DAILY_SERVICE_TIMES'
  | 'READ_DUPLICATE_APPLICATIONS'
  | 'READ_INCOME_STATEMENTS'
  | 'READ_PEDAGOGICAL_DOCUMENTS'
  | 'READ_PLACEMENT'
  | 'READ_PLACEMENT_STATUS_BY_APPLICATION_TYPE'
  | 'READ_SERVICE_APPLICATIONS'
  | 'READ_SERVICE_NEEDS'

export type Decision =
  | 'DOWNLOAD_PDF'
  | 'READ'

export type ServiceApplication = 'DELETE'

}

}
